...
- **Prefix XOR:** Compute the XOR sum of all elements from $0$ to $a - 1$. Since $a$ can be up to $3 \cdot 10^5$, calculating this naively for every test case would result in TLE. We can compute the prefix XOR in $\mathcal{O}(1)$ time using the cyclic property of numbers modulo 4.
